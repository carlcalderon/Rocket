...
| :----------- | :-------- | :-------------------------------------------------------------------------------------- |
| \--schematic | -s        | Outputs a schematic describing the build order & output from the specified config file. |
| \--approve   | -a        | Automatically approves the schematic.                                                   |
| \--watch     | -w        | Starts watch mode after first successful build.                                         |
...
The configuration file in this case, would exist at the root of the project in the same folder as the source directory.

The second thing to notice is that the objects in *build* object do not share the same setup:

1. First we have the index.html file which will be parsed for notation blocks and copied directly.
2. Next we have the images directory which will have all its contents copied over to the output folder.
3. We then have a single .coffee file which is **compiled** and **minified** into an outputted scripts.js.
4. Finally, we have two separate less files that are compiled into styles.css in the output directory.
