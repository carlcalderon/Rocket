...
(function(output){
    ...
    function parse(filepath, program) {
        ...
        if (data.build != null) {
            ...
            if (util.isArray(data.build)) {
                ...
                buildOrders.push(parseBuildOrder({id:"build", items:data.build}, inputDir, outputDir, defaultBuildOrder));
                ...
                if (util.isArray(data.buildOrders)) {
                    ...
                    if (data.buildOrders.length > 0) {
                        ...
                        for (i = 0, len = data.buildOrders.length; i < len; i++) {
                            ...
                            // Push parsed build order to buildOrders list
                            buildOrders.push(parseBuildOrder(data.buildOrders[i], inputDir, outputDir, defaultBuildOrder));
    ...
    function parseBuildOrder(input, inputDir, outputDir, targetBuildOrder) {
            ...
            if (util.isArray(input.items)) {
                ...
                if (input.items.length > 0) {
                    ...
                    for (var i = 0, len = input.items.length; i < len; i++) {
                        ...
                        items.push(parseBuildItem(input.items[i], inputDir, outputDir, targetBuildOrder));
    ...
    function parseBuildItem(item, inputDir, outputDir, targetBuildOrder) {
            ...
            if (isAssumedFile(item.output)) {
                ...
                for ( ; i < len; i++) {
                    ...
                    if (notation.match(inputPath)) {
                        ...
                        notationResult = notation.parse(filecontent, inputPath, 1024, targetBuildOrder);
        ...
        if (!!item.hooks) {
            ...
            ["before", "after"].forEach(function (field) {
                ...
                var orders = item.hooks[field];

                if (!!orders) {

                    hooks[field] = {
                        type: itemTypes.BUILD_ORDER,
                        errors: [],
                        warnings: [],
                        notation: [],
                        buildOrder: [],
                    ...
                    for (var i = 0, len = orders.length; i < len; i++) {
                        ...
                        if (buildOrderIDs.indexOf(orders[i]) == -1) {

                            result.warnings.push({
                                id: warnings.INVALID_BUILD_ORDER_ID,
                                reason: "No Build Order with id \"" + orders[i] + "\" found, ignored."
                            });

                        } else {

                            hooks[field].buildOrder.push(orders[i]);
